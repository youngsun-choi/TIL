...
public class NewsServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action.equals("insert")) { //입력
			...
			for(NewsVO list:searchList) {
				if(list!=null){
					request.setAttribute("list", dao.search(keyword, searchType));
					request.setAttribute("homebtn", "homebtn");
					//request.setAttribute("msg", title + "찾았다아아");
					request.getRequestDispatcher("/jspexam/news.jsp").forward(request, response);
					return;
				}else if(list==null){
					request.setAttribute("msg", "찾으시는 내용이 없습니다.");
					return;
				}
			}
			/*
			 * if(keyword.equals("") || dao.search(keyword, searchType)==null) {
			 * request.setAttribute("msg", "찾으시는 내용이 없습니다."); }else if(searchList!=null){
			 * request.setAttribute("list", dao.search(keyword, searchType));
			 * request.setAttribute("homebtn", "homebtn"); //request.setAttribute("msg",
			 * title + "찾았다아아");
			 * request.getRequestDispatcher("/jspexam/news.jsp").forward(request, response);
			 * return; }
			 */
